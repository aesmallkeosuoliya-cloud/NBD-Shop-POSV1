...
import { getInternalUsers, addInternalUser, updateInternalUser, deleteInternalUser } from '../../services/firebaseService';
...
const UserManagementPage: React.FC = () => {
  ...
  const fetchUsers = useCallback(async () => {
    setIsLoading(true);
    try {
      const fetchedUsers = await getInternalUsers();
  ...
  const handleDeleteUser = async (user: InternalUser) => {
    ...
    if (result.isConfirmed) {
      try {
        await deleteInternalUser(user.id, currentUser!.id, currentUser!.login);
  ...
  const handleSubmitForm = async (userData: Partial<InternalUser>, password?: string) => {
    setFormLoading(true);
    try {
      if (editingUser) {
        await updateInternalUser({ ...editingUser, ...userData } as InternalUser, password, currentUser!.id, currentUser!.login);
      } else {
        await addInternalUser(userData, password!, currentUser!.id, currentUser!.login);
      }
      Swal.fire(t('success'), t('saveSuccess'), 'success');
      setIsModalOpen(false);
      fetchUsers();
    } catch (error: any) {
        if (error.message.includes('login_exists')) {
  ...
  const exportToPDF = () => {
    ...
    (doc as any).autoTable({
        head: [[t('login'), t('role'), 'Created At']],
        body: filteredUsers.map(u => [u.login, t(`role_${u.role}`), new Date(u.createdAt).toLocaleDateString()]),
        startY: 20,
    });
  ...
  return (
    <div className="p-4 md:p-6 h-full flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <Button onClick={handleAddUser} variant="primary" leftIcon={<PlusIcon />}>{t('addNewUser')}</Button>
      </div>
        ...
        <div className="overflow-auto flex-grow">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500">{t('login')}</th>
      ...
      {isModalOpen && (
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingUser ? t('editUser') : t('addNewUser')}>
